        ...
        <div class="col-xs-12">
            ...
            <div class="box box-info">
                <div class="box-header with-border">
                    <h3 class="box-title">编辑数据表</h3>
                </div>
                <!-- /.box-header -->
                <!-- form start -->
                <form class="form-horizontal" method="post" action="{{route('sys-table.update', $entity->id)}}">
                    {!! csrf_field() !!}
                    {!! method_field('PUT') !!}
                    <div class="box-body">
                            ...
                            <div class="col-sm-6">
                                <select class="form-control selectpicker" id="sys_module_id" name="sys_module_id">
                                    ...
                                    @forelse($modules as $mod)
                                        <option value="{{$mod->id}}" {{$entity->sys_module_id == $mod->id ? 'selected':''}} data-content="<span class=''>{{$mod->name}}</span>"></option>
                                    @empty
                                    @endforelse
                                </select>
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-6">
                                <input type="text" class="form-control" id="name" placeholder="名称" name="name" value="{{$entity->name}}">
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="desc" class="col-sm-2 control-label">描述</label>
                            <div class="col-sm-6">
                                <input type="text" class="form-control" id="desc" placeholder="描述" name="desc" value="{{$entity->desc}}">
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-6">
                                <select class="form-control selectpicker" id="icon" name="icon">
                                    ...
                                    @forelse($icons as $icon)
                                        <option value="{{$icon->value}}" {{$entity->icon == $icon->value ? 'selected':''}} data-content="<span class='{{$icon->value}}'></span>"></option>
                                        @empty
                                    @endforelse
                                </select>
                            </div>
                    ...
                    <div class="box-footer">
                        <a href="{{route('sys-table.index')}}" class="btn btn-default">返回</a>
